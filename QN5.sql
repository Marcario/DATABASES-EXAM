...
-- qn5a(i) selecting students with Reg_no,last_name,and first_name
SELECT Reg_no,Last_Name,First_name FROM students;

-- ii)

 SELECT *FROM Students
INNER JOIN Last_name ON first_name;
...
UPDATE Last_name
SET Reg_no=4/2017 TO  'Mwagi';


DELETE FROM Students;


b)WHERE It is used to extract only those records that fulfill a specified condition.

GROUP BY It is statement is often used with aggregate functions (COUNT(), MAX(), MIN(), SUM(), AVG()) to group the result-set by one or more columns.

DISTINCT  is a statement is used to return only distinct (different) values.
